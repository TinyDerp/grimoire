...
import { clearAllTrippySkins, listAppliedTrippySkins } from '../services/trippyEffects';
...
ipcMain.handle('get-locker-overview', async (): Promise<LockerOverview> => {
    ...
    if (!deadlockPath) return { cards: [], sounds: [], colors: [], trippySkins: [] };
    const [cards, sounds] = await Promise.all([
        listAppliedCards(deadlockPath),
        listAppliedSounds(deadlockPath),
    ]);
    // Colors and trippy skins come from their metadata manifests (synchronous).
    const colors = listAppliedColors();
    const trippySkins = listAppliedTrippySkins();
    return { cards, sounds, colors, trippySkins };
});
...
ipcMain.handle(
    ...
    async (_, scope: LockerClearScope): Promise<void> => {
        ...
        if (scope === 'trippy' || scope === 'all') await clearAllTrippySkins(deadlockPath);
    },
);
